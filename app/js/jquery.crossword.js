			...
			*/
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {
						...
						for (var x=0, j = puzz.data[i].answer.split(';').length; x < j; ++x) { // CHANGED BY HS
							entries[i].push(x);
				...
				*/
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						var letters = puzz.data[x-1].answer.split(';'); // CHANGED BY HS
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							if($(light).empty()){
								$(light)
									...
									.append('<input val="" type="text" tabindex="-1" />'); /*maxlength="1"  */
									// .append('<span class="answercell"></span>');
							}
						};
					...
					/*for (var i=1, p = entryCount; i < p; ++i) {
						...
						if(!$('.entry-' + i +':eq(0) span').length){
							console.log('puzz.data[i].position :::' + puzz.data[i].position);
							$groupedLights.eq(0)
			...
			var nav = {
				...
				updateByEntry: function(e, next) {
					...
					if(e.keyCode === 9 || next){
						...
						activeClueIndex = activeClueIndex === clueLiEls.length-1 ? 0 : ++activeClueIndex;
						console.log('1. activeClueIndex ::' + activeClueIndex);

						util.getActivePositionFromClassGroup(e.target);

						clue = $(clueLiEls + '[data-position=' + activePosition + ']');
						console.log('1.1. clue ::' + clue);
						activeClueIndex = $(clueLiEls).index(clue);
						console.log('2. activeClueIndex ::' + activeClueIndex);

						currOri = clue.parent().prop('id');
						console.log('3. currOri ::' + currOri);
					}
			...
			var util = {
				highlightEntry: function() {
					...
					console.log('Inside highlightEntry');
				...
				getClasses: function(light, type) {
					...
					console.log('getClasses positions :::' + positions);
					return positions;
				},
				...
				getActivePositionFromClassGroup: function(el){
						...
						console.log('getActivePositionFromClassGroup activePosition: '+activePosition + '--' + puzz.data[activePosition].answer);

						// HS - Show letters for the answer
						$('#entry-clue').html(puzz.data[activePosition].clue);
						$('#entry-answer-letters').html(puzz.data[activePosition].answer.split(';').join('  '));
						// puzz.data[i].answer
